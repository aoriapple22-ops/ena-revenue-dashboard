...
export async function fetchAll() {
  ...
  return {
    ...
    // 드라마는 뒤늦게 추가된 테이블이라, 없어도 다른 화면이 깨지지 않게 error에 포함하지 않는다
    drama: drama.data || [],
...
// 월별 추이에서 빼는 매출구분.
// 인서트애드는 채널 매출이 아니라 건별 계약 금액이라 월별 흐름을 왜곡한다.
// 요약 그래프와 디지털 월별 표가 같은 기준을 쓰도록 여기서 한 번만 정의한다.
export const MONTHLY_EXCLUDED_TYPES = ["공동매출(뉴미디어)"];
export const MONTHLY_EXCLUDED_LABEL = "공동매출(뉴미디어·인서트애드)";

export function excludeMonthlyTypes(rows) {
  return rows.filter((r) => !MONTHLY_EXCLUDED_TYPES.includes(r.revenue_type));
}
